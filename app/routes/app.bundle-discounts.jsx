...
/**
 * IMPORTANT:
 * Replace this once with your real Shopify Function ID.
 * This is ONLY needed to create the automatic app discount the first time.
 */
const FUNCTION_ID = "REPLACE_WITH_YOUR_FUNCTION_ID";
...
function normalizeConfig(rawConfig) {
  ...
  return {
    rules: rawConfig.rules.map((rule) => ({
      ...
      accessories: Array.isArray(rule?.accessories) && rule.accessories.length > 0
      ...
      .map((rule) => ({
        ...
        accessories: (rule.accessories || [])
          .map((accessory) => ({
            ...
            discountAmount: parseFloat(String(accessory.discountAmount || "").replace(",", ".")),
...
async function findOrCreateBundleDiscount(admin) {
  ...
  if (!FUNCTION_ID || FUNCTION_ID === "REPLACE_WITH_YOUR_FUNCTION_ID") {
    throw new Error("Set FUNCTION_ID in app.bundle-discounts.jsx before first use.");
  }
